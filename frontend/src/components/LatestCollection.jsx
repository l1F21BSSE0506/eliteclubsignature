  ...
  return (
    <div className='my-10'>
      <div className='text-center py-8'>
        <Title text1={'NEW'} text2={'IN'} />
        <p className='w-3/4 m-auto text-xs sm:text-sm text-gray-500 uppercase tracking-widest'>
          Explore the latest arrivals and elevate your style.
        </p>
      </div>
      {/* Rendering products */}
      <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 gap-y-6'>
        {latestProducts.map((item, index) => (
          <ProductItem key={index} id={item._id} image={item.images} name={item.name} price={item.price} discount={item.discount} />
        ))}
      </div>
    </div>
  );
